...
namespace adventofcode2017
{
    ...
    public class Day24Part2
    {
        ...
        public void TestRun()
        {
            ...
            Assert.AreEqual(19, new Program().RunChallenge(input));
        }
        ...
        public class Program
        {
            ...
            public long RunChallenge(List<string> input)
            {
                ...
                start.ForEach(z => {
                    ...
                    FindNextComponent(z.PortA == 0 ? z.PortB : z.PortA, componentsNotUsed, weight, 1);
                });

                return Strongest;
            }

            private void FindNextComponent(int portToMatch, List<Component> ComponentsNotUsedFromStart, int bridgeWeight, int length)
            {
                var nextComponent = ComponentsNotUsedFromStart.Where(z => z.PortA == portToMatch || z.PortB == portToMatch).ToList();

                if(!nextComponent.Any() && length >= Longest)
                {
                    if (length > Longest)
                    {
                        Strongest = bridgeWeight;
                    } else if ((length == Longest) && (bridgeWeight > Strongest))
                    {
                        Strongest = bridgeWeight;
                    }
                    Longest = length;
                ...
                nextComponent.ForEach(z => {
                    ...
                    FindNextComponent(otherPort, componentsNotUsed, newWeight, newLength);
                });
            }
        }
    }
